...
test("server-renders the ParticlePair product shell", async () => {
  ...
  assert.match(html, /VALIDATE ALL 13 VISUAL MODES/);
  assert.match(html, /RUN CAMERA STRESS SUITE/);
  assert.match(html, /AUDIT ALL 13 UNIQUE VISUALS/);
  assert.match(html, /ARCHITECT SIGNAL/);
  assert.match(html, /UNIVERSAL CHANNEL ATLAS/);
  assert.match(html, /EXPORT LOCAL DIAGNOSTIC/);
  assert.match(html, /aria-live="polite"/);
  assert.match(html, /Spiral Galaxy/);
  assert.match(html, /Ember Transfer/);
  assert.doesNotMatch(html, /Laser Weave/);
