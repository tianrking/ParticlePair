...
export function drawArtisticScene({ context, height, mode, pixelRatio, time, width }: ArtisticSceneOptions): void {
  ...
  if (mode.kind !== "galaxy") {
    const spectralLobes: readonly [number, number, string][] = [[-0.23, -0.18, a], [0.25, -0.04, b], [0.02, 0.25, c]];
  ...
  if (["nodes", "clouds", "city", "facets", "orbits"].includes(mode.kind)) {
    for (let index = 0; index < 24; index += 1) {
      const angle = index * 2.399 + t * (0.18 + (index % 5) * 0.025);
  ...
  if (mode.kind === "curtain") {
    ...
    for (let i = 1; i <= 6; i += 1) {
      const radius = side * (0.07 + i * 0.06); ring(context, cx, cy, radius, color([a, b, c][i % 3], 0.28), pixelRatio * 1.2);
      const angle = t * (0.13 + i * 0.027) + i * 1.7; context.beginPath(); context.arc(cx + Math.cos(angle) * radius, cy + Math.sin(angle) * radius, side * (0.007 + i * 0.0016), 0, Math.PI * 2); context.fillStyle = color([a, b, c][i % 3], 0.92); context.fill();
    }
    ...
    context.strokeStyle = color(b, 0.18); context.lineWidth = pixelRatio * 1.25;
    points.forEach((point, i) => { const next = points[(i * 7 + 5) % points.length]; context.beginPath(); context.moveTo(point.x, point.y); context.lineTo(next.x, next.y); context.stroke(); context.beginPath(); context.arc(point.x, point.y, (2.4 + i % 3) * pixelRatio, 0, Math.PI * 2); context.fillStyle = color([a, b, c][i % 3], 0.9); context.fill(); });
    ...
    for (let ringIndex = 1; ringIndex <= 5; ringIndex += 1) for (let i = 0; i < 12; i += 1) { const r0 = side * ringIndex * 0.075; const r1 = r0 + side * 0.07; const p0 = i * Math.PI / 6; const p1 = p0 + Math.PI / 6; context.beginPath(); context.moveTo(Math.cos(p0) * r0, Math.sin(p0) * r0); context.lineTo(Math.cos(p0) * r1, Math.sin(p0) * r1); context.lineTo(Math.cos(p1) * r1, Math.sin(p1) * r1); context.closePath(); context.fillStyle = color([a, b, c][(i + ringIndex) % 3], 0.14 + ((i + ringIndex) % 3) * 0.055); context.fill(); }
  } else if (mode.kind === "glyph") {
    for (let i = 1; i <= 7; i += 1) { ring(context, cx, cy, side * i * 0.055, color([a, b, c][i % 3], 0.36), pixelRatio * (i % 2 ? 1.2 : 2.4)); for (let j = 0; j < i * 3; j += 1) { const angle = j * Math.PI * 2 / (i * 3) + t * (i % 2 ? 0.025 : -0.02); context.fillStyle = color([a, b, c][j % 3], 0.68); context.fillRect(cx + Math.cos(angle) * side * i * 0.055 - pixelRatio, cy + Math.sin(angle) * side * i * 0.055 - pixelRatio, pixelRatio * 2.4, pixelRatio * 2.4); } }
