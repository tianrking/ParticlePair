...
export function drawArtisticScene({ context, height, mode, pixelRatio, time, width }: ArtisticSceneOptions): void {
  ...
  if (mode.kind === "curtain") {
    ...
    const legacyStardust = mode.id === "stardust"; const emberTime = legacyStardust ? time * 0.001 : t * 0.42;
    ...
    const emberCount = legacyStardust ? 80 : 96;
    for (let i = 0; i < emberCount; i += 1) { const life = (emberTime * (legacyStardust ? 0.08 + i % 5 * 0.012 : 0.095 + variant * 0.055 + i % 5 * 0.012) + i * 0.071) % 1; const x = legacyStardust ? cx + Math.sin(i * 8.31 + life * 7) * side * (0.06 + life * 0.24) : cx + Math.sin(i * (6.8 + variant * 3) + life * (5 + variant * 5)) * side * (0.04 + variant * 0.04 + life * (0.16 + variant * 0.13)); const y = cy + side * 0.31 - life * side * (legacyStardust ? 0.72 : 0.64 + variant * 0.16); context.beginPath(); context.arc(x, y, pixelRatio * (legacyStardust ? 1 + i % 3 : 1.25 + variant + (i % 3) * 1.1), 0, Math.PI * 2); context.fillStyle = color([a, b, c][i % 3], (1 - life) * (legacyStardust ? 0.72 : 0.78)); context.fill(); }
    if (!legacyStardust) for (let mote = 0; mote < 70; mote += 1) { const x = cx + Math.sin(mote * (31.7 + variant * 9)) * side * (0.08 + (mote % 9) * 0.038); const y = cy + side * 0.3 - ((mote * 0.137 + variant) % 1) * side * (0.58 + variant * 0.18); context.beginPath(); context.arc(x, y, pixelRatio * (1.1 + mote % 3), 0, Math.PI * 2); context.fillStyle = color([a, b, c][mote % 3], 0.38 + (mote % 4) * 0.08); context.fill(); }
