...
export function drawArtisticScene({ context, height, mode, pixelRatio, time, width }: ArtisticSceneOptions): void {
  ...
  if (mode.kind === "curtain") {
    ...
    context.beginPath(); context.ellipse(cx, cy - side * 0.12, side * 0.19, side * 0.13, 0, 0, Math.PI * 2); context.fillStyle = color(a, legacyAnemone ? 0.12 : 0.19); context.fill();
    const tendrilCount = legacyAnemone ? 14 : 10 + Math.floor(variant * 9); const flowTime = legacyAnemone ? time * 0.001 : t;
    for (let i = 0; i < tendrilCount; i += 1) { context.beginPath(); const x = cx + (legacyAnemone ? (i - 6.5) * side * 0.026 : (i - (tendrilCount - 1) / 2) * side * (0.32 / tendrilCount)); context.moveTo(x, cy - side * 0.02); context.bezierCurveTo(x + Math.sin(flowTime + i) * side * (legacyAnemone ? 0.06 : 0.04 + variant * 0.04), cy + side * 0.12, x + Math.cos(flowTime * 0.7 + i) * side * 0.08, cy + side * 0.27, x + Math.sin(flowTime * 0.4 + i) * side * (legacyAnemone ? 0.05 : 0.035 + variant * 0.04), cy + side * 0.42); context.strokeStyle = color([a, b, c][i % 3], legacyAnemone ? 0.46 : 0.56); context.lineWidth = pixelRatio * (legacyAnemone ? 2.5 : 1.8 + variant * 1.4); context.stroke(); }
