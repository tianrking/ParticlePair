...
- The deterministic Visual Quality Engine renders a canonical carrier, measures vibrancy, contrast, hue coverage, motion continuity, and a composite grade, then compares an 8×8 color-motion fingerprint against the nearest mode in the same renderer family. The current 50-mode baseline is 64 minimum, 73 average, and 49 minimum distinctness; the audit fails below 60 quality or 40 distinctness.
...
- Environmental Mode Oracle renders all 50 modes through a selected synthetic camera channel and ranks measured outcomes. Exact secret recovery plus CRC is mandatory; passing modes are then ordered by synchronization quality and Hamming repair count, with deterministic catalog order only as a final tie-breaker. The top three can be applied directly.
- Universal Channel Atlas executes every one of the 50 renderers across all six synthetic camera channels—300 real-pixel paths. Its minimax ranking prioritizes complete CRC coverage, then the weakest channel quality, mean quality, and Hamming repair cost. A 6×50 spectral field exposes brittle modes while the three most universally robust renderers can be applied immediately.
...
- Reliability Marathon runs a fixed corpus of eight unique 128-bit secrets through all 50 renderers: 400 real two-phase Canvas assessments with exact recovery, Hamming, and CRC. Its 8×50 heatmap, pass count, and minimum sync floor are reproducible across browsers. Batch tools use the identical pixel sampler and decoder while skipping PNG evidence encoding; the single-case proof still produces full images.
- Evidence Seal exports a canonical `particlepair-reliability/v1` JSON record containing the 400 rounded quality outcomes, configuration, summary, explicit privacy flags, and a SHA-256 digest. Verification detects file modification. The seal is deliberately labeled integrity-only: it is neither a signature, trusted timestamp, device attestation, nor proof that an independent party ran the test.
