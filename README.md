...
Occlusion guidance is orientation-aware. The weakest canonical payload quadrant is mapped back through the winning rotation/mirror transform into the camera-observed top-left, top-right, bottom-left, or bottom-right. The corresponding cyan guide corner becomes a pulsing magenta nebula marker and the status text names that observed corner. All eight transforms remain bijective, so the user acts on the corner they actually see rather than an internal decoded coordinate.
...
- 13 selectable visual modes across Cosmic, Organic, Geometric, Atmospheric, and Synthetic collections—exactly one representative for each renderer family.
- 13 structurally independent renderers: spiral field, curtain, rings, petals, orbits, node graph, tendrils, clouds, weave, facets, glyph, city grid, and embers. Historical palette/parameter variants are deliberately excluded from the public catalog instead of being counted as new modes.
...
- The deterministic Visual Quality Engine renders a canonical carrier and measures vibrancy, contrast, hue coverage, motion continuity, and a composite grade. Catalog integrity additionally requires exactly one mode per renderer family, preventing palette or parameter variants from reappearing as nominally new visuals.
...
- Visual DNA hashes the complete optical frame into a deterministic art seed. Each payload receives a repeatable constellation topology, orbit bias, chroma phase, eight-hex visual checksum, and human-readable identity such as `AURORA GATE`; it is explicitly decorative and not an authenticator. The sparse constellation is common-mode across both optical phases and needs no camera-specific feature.
- Mission Control balances the long transmitter laboratory with an independently sticky left-side control rail: live visual identity and palette, Generate → Transmit → Verify flow, current 13/104/78 engineering scope, optical protocol stack, 13 interactive renderer shortcuts, and an explicit local-first privacy statement. The narrative heading scrolls away while the control rail stays at 94px; below 1050px its deep controls collapse and the card returns to normal flow.
- Adaptive Motion Engine measures real animation cadence and uses asymmetric hysteresis to move between Ultra, Balanced, and Efficient decorative budgets. It downgrades quickly under sustained pressure, restores slowly to avoid visible flapping, ignores background-tab gaps, and never reduces the full-resolution optical carrier. Manual overrides remain available.
- Optical Confidence Aurora maps five independent receiver layers—capture health, synchronization, geometry consensus, evidence diversity, and payload coverage—into a live spectral diagnostic. Its weighted score never replaces CRC; it highlights the weakest layer and distinguishes acquisition, alignment, collection, degraded, and integrity-ready states.
- Studio Capsule serializes only allowlisted presentation settings—mode, protocol version, v2 dwell, modulation strength, and render budget—into a versioned shareable URL. Decoding rejects unknown modes and out-of-range values; URL generation strips every existing query parameter and fragment, so secrets, session IDs, SAS values, and camera data never enter the capsule.
- Environmental Mode Oracle renders all 13 structurally unique modes through a selected synthetic camera channel and ranks measured outcomes. Exact secret recovery plus CRC is mandatory; passing modes are then ordered by synchronization quality and Hamming repair count, with deterministic catalog order only as a final tie-breaker. The top three can be applied directly.
- Universal Channel Atlas executes every one of the 13 renderers across all six synthetic camera channels—78 real-pixel paths. Its minimax ranking prioritizes complete CRC coverage, then the weakest channel quality, mean quality, and Hamming repair cost. A 6×13 spectral field exposes brittle modes while the three most universally robust renderers can be applied immediately.
- Phase-Safe Choreography aligns automatic scene changes to complete 600 ms A+B boundaries, confining decorative discontinuity to a known pair boundary instead of switching arbitrarily inside a symbol. Phase Chronograph exposes the live A/B phase and remaining milliseconds through direct DOM updates, avoiding page-level React rerenders at animation frequency.
- Immersive Receiver provides a full-screen, one-handed camera surface with the same underlying scanner and a mirrored preview stream. It combines an occlusion-aware corner guide, compact Confidence Aurora, bottleneck-specific physical instructions, Escape/visible close controls, and optional Wake Lock. The feature opens even before permission so camera access remains an explicit user action.
- Lab Dock consolidates compatibility, camera-channel, environment-oracle, and aesthetic audits into one four-way segmented workspace. Inactive modules remain mounted so completed evidence is preserved without recomputation; navigation locks while any experiment runs, preventing hidden or ambiguous progress.
- Reliability Marathon runs a fixed corpus of eight unique 128-bit secrets through all 13 renderers: 104 real two-phase Canvas assessments with exact recovery, Hamming, and CRC. Its 8×13 heatmap, pass count, and minimum sync floor are reproducible across browsers.
- Evidence Seal exports a canonical `particlepair-reliability/v2` JSON record containing the 104 rounded quality outcomes, configuration, summary, explicit privacy flags, and a SHA-256 digest. V2 deliberately identifies the deduplicated 8×13 corpus and cannot be confused with legacy 8×50 evidence.
- Local Evidence Inspector accepts that JSON directly in the browser, enforces the exact 104-case corpus/schema/privacy constraints, recomputes the summary and canonical SHA-256 digest, and clearly separates verified, modified, and malformed records.
- Reliability Constellation compares two independently verified records cell by cell. A two-percentage-point deadband suppresses rounding noise; exact pass/fail transitions always win. The 104-cell spectral map reports improved, stable, and regressed cases.
