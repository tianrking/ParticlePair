...
“Dos fotogramas” significa dos imágenes fuente independientes, no dos callbacks de JavaScript. Cada depósito de geometría/orientación rechaza un `video.currentTime` positivo sin cambios aunque vuelva a dispararse `requestAnimationFrame`. Si el tiempo multimedia no existe o permanece en cero, se usa el 72% del intervalo medido, limitado a 18–55 ms. Las tres luces `EVIDENCE` cian, violeta y magenta muestran la acumulación independiente; callbacks duplicados no encienden otro punto ni avanzan la decodificación.

La confianza por celda del combinador suave es ahora una puerta de decodificación. Se analizan las 252 celdas codificadas, la cobertura bajo el umbral 0.32, todas las ventanas móviles 4×4 y la densidad débil en cuatro cuadrantes. Debilidad aislada sigue siendo decodificable; menos de 72% de cobertura o una ventana local 75% débil bloquea el proceso. Una ventana 62.5% débil o un cuadrante degradado se etiqueta como oclusión, separada de cobertura difusa. `Q` junto a `EVIDENCE` muestra cobertura y cambia el medidor entre verde, ámbar y magenta nebulosa con instrucciones concretas.

La indicación de oclusión respeta la orientación. El cuadrante canónico más débil se transforma mediante la rotación/reflejo ganador hasta la esquina observada superior izquierda, superior derecha, inferior izquierda o inferior derecha. Esa esquina cian se convierte en un marcador magenta nebuloso pulsante y el texto nombra la esquina visible. Las ocho transformaciones mantienen una correspondencia biyectiva, de modo que el usuario corrige la esquina que ve y no una coordenada interna.
...
ParticlePair separa la portadora óptica fiable de la capa artística generativa. Los 13 renderizadores estructuralmente únicos comparten la misma portadora diferencial calibrada de 18×18, por lo que cambiar el aspecto no exige otro decodificador de cámara.

- 13 modos en cinco colecciones, exactamente un representante por familia de renderizado.
- 13 renderizadores estructuralmente independientes: galaxia, cortina, anillos, pétalos, órbitas, nodos, tentáculos, nubes, tejido, facetas, glifo, ciudad y brasas. Las variantes históricas de paleta o parámetros quedan fuera del catálogo público.
...
- Visual Quality Engine usa un portador canónico determinista y mide viveza, contraste, cobertura cromática, continuidad de movimiento y nota compuesta. La integridad del catálogo exige además un único modo por familia para impedir que cambios de color o parámetros vuelvan como modos supuestamente nuevos.
...
- Visual DNA convierte el fotograma óptico completo en una semilla artística determinista. Cada carga obtiene una topología de constelación repetible, sesgo orbital, fase cromática, suma visual hexadecimal y una identidad legible como `AURORA GATE`. Es decoración, no autenticación; la constelación dispersa es común a ambas fases y no exige funciones especiales de cámara.
- Mission Control equilibra el laboratorio con un carril de control independiente y fijo: identidad visual, flujo Generar → Transmitir → Verificar, alcance 13/104/78, pila óptica, 13 accesos interactivos y privacidad local. El título narrativo se desplaza mientras el carril queda a 94px; por debajo de 1050px oculta los controles profundos y vuelve al flujo vertical.
- Adaptive Motion Engine mide la cadencia real y usa histéresis asimétrica para alternar entre presupuestos Ultra, Balanced y Efficient. Reduce pronto bajo presión sostenida, recupera lentamente para evitar saltos visibles, ignora pausas de pestañas en segundo plano y nunca reduce el portador óptico a resolución completa. También conserva controles manuales.
- Optical Confidence Aurora convierte cinco capas independientes del receptor —salud de captura, sincronización, consenso geométrico, diversidad de evidencia y cobertura de carga— en un diagnóstico espectral en vivo. Su nota ponderada nunca sustituye al CRC: destaca la capa limitante y diferencia adquisición, alineación, recopilación, degradación y estado listo con integridad.
- Studio Capsule serializa únicamente ajustes de presentación permitidos —modo, versión del protocolo, intervalo v2, fuerza de modulación y presupuesto de render— en una URL versionada. El decodificador rechaza modos desconocidos y valores fuera de rango; la URL elimina todos los parámetros y fragmentos existentes, por lo que secretos, sesiones, SAS y datos de cámara nunca entran en la cápsula.
- Environmental Mode Oracle renderiza los 13 modos estructuralmente únicos a través del canal sintético elegido y clasifica los resultados medidos.
- Universal Channel Atlas ejecuta los 13 renderizadores a través de seis canales: 78 rutas de píxeles reales. Su clasificación minimax prioriza cobertura CRC completa, calidad del canal más débil, calidad media y coste de reparación Hamming.
- Phase-Safe Choreography alinea los cambios automáticos con límites A+B completos de 600 ms, confinando la discontinuidad decorativa a un límite conocido en vez de cambiar dentro de un símbolo. Phase Chronograph muestra fase A/B y milisegundos restantes mediante actualizaciones DOM directas, sin rerenderizar toda la página a frecuencia de animación.
- Immersive Receiver ofrece una superficie de cámara a pantalla completa para una sola mano, reutilizando el mismo escáner y reflejando el flujo solo para vista previa. Combina esquinas sensibles a oclusión, Confidence Aurora compacto, instrucciones físicas según el cuello de botella, Escape/cierre visible y Wake Lock opcional. Puede abrirse antes del permiso, por lo que iniciar la cámara sigue siendo una acción explícita.
- Lab Dock reúne compatibilidad, canal de cámara, Oracle ambiental y auditoría estética en un espacio segmentado de cuatro vías. Los módulos inactivos permanecen montados para conservar resultados sin recalcular; la navegación se bloquea durante cualquier experimento para evitar progreso oculto o ambiguo.
- Reliability Marathon ejecuta ocho secretos en los 13 renderizadores: 104 evaluaciones Canvas bifásicas. El mapa 8×13 y el mínimo de sincronización son reproducibles entre navegadores.
- Evidence Seal exporta `particlepair-reliability/v2` con 104 resultados y SHA-256. V2 identifica explícitamente el corpus deduplicado 8×13 y no se confunde con evidencia 8×50 antigua.
- Local Evidence Inspector exige las 104 celdas, su esquema y las restricciones de privacidad, y recalcula localmente el resumen y SHA-256.
- Reliability Constellation compara dos registros verificados de 104 celdas con una zona muerta de dos puntos porcentuales.
