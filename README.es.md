...
- Visual Quality Engine usa un portador canónico determinista, mide viveza, contraste, cobertura cromática, continuidad de movimiento y nota compuesta, y compara una huella 8×8 de color y movimiento con el modo más cercano de la misma familia. La base actual de 50 modos es 64 mínimo, 73 de media y 49 de distinción mínima; la auditoría falla por debajo de 60 de calidad o 40 de distinción.
...
- Environmental Mode Oracle renderiza los 50 modos a través del canal de cámara sintético elegido y clasifica los resultados medidos. La recuperación exacta y el CRC son obligatorios; después se ordena por calidad de sincronización y número de reparaciones Hamming, usando el catálogo solo como desempate final. Los tres mejores se pueden aplicar directamente.
- Universal Channel Atlas ejecuta los 50 renderizadores a través de los seis canales sintéticos de cámara: 300 rutas de píxeles reales. Su clasificación minimax prioriza cobertura CRC completa, calidad del canal más débil, calidad media y coste de reparación Hamming. El campo espectral 6×50 revela modos frágiles y permite aplicar de inmediato los tres renderizadores más robustos.
...
- Reliability Marathon ejecuta un corpus fijo de ocho secretos únicos de 128 bits en los 50 renderizadores: 400 evaluaciones Canvas bifásicas con recuperación exacta, Hamming y CRC. El mapa 8×50, total aprobado y mínimo de sincronización son reproducibles entre navegadores. Las herramientas masivas omiten solo la codificación PNG; conservan el mismo muestreador y decodificador.
- Evidence Seal exporta un JSON canónico `particlepair-reliability/v1` con los 400 resultados redondeados, configuración, resumen, indicadores explícitos de privacidad y un SHA-256. La verificación detecta modificaciones. Se etiqueta como integridad solamente: no es firma, sello temporal confiable, atestación de dispositivo ni prueba de una ejecución independiente.
