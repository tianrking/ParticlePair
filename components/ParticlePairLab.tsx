...
import { VISUAL_CATEGORIES, VISUAL_MODES, VISUAL_MODE_COUNT, visualMode as getVisualMode, type VisualCategory, type VisualModeId } from "../lib/visual-modes";
...
import { buildReliabilityEvidence, inspectReliabilityEvidence, RELIABILITY_CASE_COUNT, RELIABILITY_SECRET_COUNT, type ReliabilityEvidence } from "../lib/reliability-evidence";
...
const MISSION_COPY: Record<Language, { live: string; route: readonly [string, string, string]; routeDetail: readonly [string, string, string]; proof: string; unique: string; corpus: string; channels: string; stack: string; layers: readonly [string, string, string, string]; constellation: string; showcase: string; privacy: string }> = {
  en: { live: "LIVE VISUAL SYSTEM", route: ["GENERATE", "TRANSMIT", "VERIFY"], routeDetail: ["Ephemeral 128-bit secret", "Complementary optical phases", "Hamming repair + CRC integrity"], proof: "ENGINEERING PROOF", unique: "structurally unique renderers", corpus: "deterministic reliability cases", channels: "cross-channel pixel paths", stack: "OPTICAL PROTOCOL STACK", layers: ["ART · generative renderer", "CARRIER · 18×18 dual phase", "RECOVERY · Hamming (12,8)", "INTEGRITY · CRC-16/CCITT"], constellation: "RENDERER CONSTELLATION", showcase: "Phase-safe showcase", privacy: "Local-first · no network transport · camera frames stay on device" },
  es: { live: "SISTEMA VISUAL EN VIVO", route: ["GENERAR", "TRANSMITIR", "VERIFICAR"], routeDetail: ["Secreto efímero de 128 bits", "Fases ópticas complementarias", "Reparación Hamming + integridad CRC"], proof: "PRUEBA DE INGENIERÍA", unique: "renderizadores estructuralmente únicos", corpus: "casos deterministas de fiabilidad", channels: "rutas de píxeles multicanal", stack: "PILA DEL PROTOCOLO ÓPTICO", layers: ["ARTE · renderizador generativo", "PORTADORA · doble fase 18×18", "RECUPERACIÓN · Hamming (12,8)", "INTEGRIDAD · CRC-16/CCITT"], constellation: "CONSTELACIÓN DE RENDERIZADORES", showcase: "Muestra sincronizada", privacy: "Local primero · sin transporte de red · los fotogramas permanecen en el dispositivo" },
  zh: { live: "实时视觉系统", route: ["生成", "传输", "验证"], routeDetail: ["一次性 128 位秘密", "互补双相光学载波", "Hamming 修复与 CRC 完整性"], proof: "工程验证能力", unique: "种结构独立渲染器", corpus: "项确定性可靠性语料", channels: "条跨信道像素路径", stack: "光学协议栈", layers: ["艺术层 · 生成式渲染器", "载波层 · 18×18 互补双相", "恢复层 · Hamming (12,8)", "完整性 · CRC-16/CCITT"], constellation: "渲染器星座", showcase: "相位安全轮播", privacy: "本地优先 · 不经过网络 · 摄像头画面留在设备内" },
};
      ...
      <section className="hero" id="top" style={{ "--mode-a": selectedVisualMode.colors[0], "--mode-b": selectedVisualMode.colors[1], "--mode-c": selectedVisualMode.colors[2] } as CSSProperties}>
        <div className="hero-copy" style={{ "--mode-a": selectedVisualMode.colors[0], "--mode-b": selectedVisualMode.colors[1], "--mode-c": selectedVisualMode.colors[2] } as CSSProperties}>
          ...
          <aside className="mission-control" aria-label={missionCopy.live}>
            ...
            <div className="mission-deep"><section><span>{missionCopy.stack}</span><ol>{missionCopy.layers.map((layer, index) => <li key={layer}><i>0{index + 1}</i><strong>{layer}</strong></li>)}</ol></section><section><span>{missionCopy.constellation}</span><div>{VISUAL_MODES.map((mode) => <button type="button" key={mode.id} className={mode.id === visualMode ? "is-active" : ""} aria-pressed={mode.id === visualMode} aria-label={mode.name} title={mode.name} onClick={() => selectVisualMode(mode.id)} style={{ "--star-a": mode.colors[0], "--star-b": mode.colors[2] } as CSSProperties}><i>{mode.icon}</i></button>)}<button type="button" className={`mission-showcase ${autoShowcase ? "is-active" : ""}`} aria-pressed={autoShowcase} aria-label={missionCopy.showcase} title={missionCopy.showcase} onClick={() => setAutoShowcase((value) => !value)} style={{ "--star-a": selectedVisualMode.colors[0], "--star-b": selectedVisualMode.colors[2] } as CSSProperties}><i>↻</i></button></div></section></div>
      ...
      <section className="workspace">
        <article className="panel sender-panel">
          ...
          <section className={`mode-oracle ${oracleStatus}`} aria-label="Environmental visual mode oracle" aria-busy={oracleStatus === "running"}>
            <div className="mode-oracle-heading"><span>ENVIRONMENTAL MODE ORACLE</span><strong>{oracleStatus === "running" ? `${oracleProgress}/${VISUAL_MODE_COUNT}` : oracleResults.length ? `${oracleResults.filter((result) => result.passed).length}/${VISUAL_MODE_COUNT} CRC PASS` : "REAL PIXEL RANKING"}</strong></div>
            ...
            <button className="oracle-run" type="button" disabled={!validSecret || oracleStatus === "running" || paused} onClick={runModeOracle}>{oracleStatus === "running" ? `TESTING ${oracleProgress}/${VISUAL_MODE_COUNT} MODES…` : "FIND THE STRONGEST VISUAL"}</button>
            {oracleResults.length ? <ol>{oracleResults.slice(0, 3).map((result) => { const mode = getVisualMode(result.mode); return <li key={result.mode}><span>#{result.rank}</span><div><strong>{mode.name}</strong><small>{Math.round(result.quality * 100)}% sync · {result.corrected} repair · {result.passed ? "CRC pass" : "rejected"}</small></div><button type="button" onClick={() => selectVisualMode(result.mode)}>APPLY</button></li>; })}</ol> : <p>Runs all {VISUAL_MODE_COUNT} structurally unique renderers through the selected synthetic camera channel. Rankings require exact secret recovery and CRC.</p>}
          </section>
          <section className={`channel-atlas ${atlasStatus}`} aria-label="Universal camera channel atlas" aria-busy={atlasStatus === "running"}>
            <div className="atlas-heading"><span>UNIVERSAL CHANNEL ATLAS</span><strong>{atlasStatus === "running" ? `${atlasProgress}/${ATLAS_CASE_COUNT}` : atlasResults.length ? `${atlasObservations.filter((cell) => cell.passed).length}/${ATLAS_CASE_COUNT} CRC` : `${CAMERA_CHANNEL_PROFILES.length} CHANNELS × ${VISUAL_MODE_COUNT} UNIQUE MODES`}</strong></div>
            <button type="button" disabled={!validSecret || atlasStatus === "running" || oracleStatus === "running" || paused} onClick={runUniversalChannelAtlas}>{atlasStatus === "running" ? `MAPPING ${atlasProgress}/${ATLAS_CASE_COUNT} PIXEL PATHS…` : "MAP THE UNIVERSAL ROBUSTNESS FIELD"}</button>
            {atlasObservations.length ? <div className="atlas-map" aria-label={`${atlasObservations.filter((cell) => cell.passed).length} of ${atlasObservations.length} channel atlas cases passed`}>{CAMERA_CHANNEL_PROFILES.map((profile, row) => <div key={profile}><b>{profile.replace("white-balance", "WB").replace("motion-blur", "MOTION").toUpperCase()}</b>{VISUAL_MODES.map((mode, column) => { const cell = atlasObservations[row * VISUAL_MODE_COUNT + column]; return <i key={mode.id} className={cell ? cell.passed ? "pass" : "fail" : undefined} title={`${mode.name} · ${profile}${cell ? ` · ${Math.round(cell.quality * 100)}% · ${cell.passed ? "CRC PASS" : "REJECTED"}` : " · pending"}`} />; })}</div>)}</div> : <p>Minimax ranking protects the weakest camera condition, not the prettiest average.</p>}
          ...
          <button className="secondary-button full-width visual-audit-button" type="button" disabled={!validSecret || qualityAuditStatus === "running" || paused} onClick={runVisualQualityAudit}>{qualityAuditStatus === "running" ? `ANALYZING VISUALS ${qualityAuditProgress}/${VISUAL_MODE_COUNT}` : `AUDIT ALL ${VISUAL_MODE_COUNT} UNIQUE VISUALS`}</button>
          <div className={`visual-audit ${qualityAuditStatus}`} role="status" aria-live="polite" aria-busy={qualityAuditStatus === "running"}>
            <div className="visual-audit-heading"><span>VISUAL QUALITY ENGINE</span><strong>{qualityAuditStatus === "success" || qualityAuditStatus === "error" ? `${Math.min(...Object.values(visualGrades).map((grade) => grade.grade))} MIN · ${Math.round(Object.values(visualGrades).reduce((sum, grade) => sum + grade.grade, 0) / VISUAL_MODE_COUNT)} AVG · ${Math.min(...Object.values(visualGrades).map((grade) => grade.distinctness))} D` : "PIXEL METRICS PENDING"}</strong></div>
