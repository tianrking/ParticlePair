...
const MISSION_COPY: Record<Language, { live: string; route: readonly [string, string, string]; routeDetail: readonly [string, string, string]; proof: string; unique: string; corpus: string; channels: string; stack: string; layers: readonly [string, string, string, string]; constellation: string; privacy: string }> = {
  en: { live: "LIVE VISUAL SYSTEM", route: ["GENERATE", "TRANSMIT", "VERIFY"], routeDetail: ["Ephemeral 128-bit secret", "Complementary optical phases", "Hamming repair + CRC integrity"], proof: "ENGINEERING PROOF", unique: "structurally unique renderers", corpus: "deterministic reliability cases", channels: "cross-channel pixel paths", stack: "OPTICAL PROTOCOL STACK", layers: ["ART · generative renderer", "CARRIER · 18×18 dual phase", "RECOVERY · Hamming (12,8)", "INTEGRITY · CRC-16/CCITT"], constellation: "RENDERER CONSTELLATION", privacy: "Local-first · no network transport · camera frames stay on device" },
  es: { live: "SISTEMA VISUAL EN VIVO", route: ["GENERAR", "TRANSMITIR", "VERIFICAR"], routeDetail: ["Secreto efímero de 128 bits", "Fases ópticas complementarias", "Reparación Hamming + integridad CRC"], proof: "PRUEBA DE INGENIERÍA", unique: "renderizadores estructuralmente únicos", corpus: "casos deterministas de fiabilidad", channels: "rutas de píxeles multicanal", stack: "PILA DEL PROTOCOLO ÓPTICO", layers: ["ARTE · renderizador generativo", "PORTADORA · doble fase 18×18", "RECUPERACIÓN · Hamming (12,8)", "INTEGRIDAD · CRC-16/CCITT"], constellation: "CONSTELACIÓN DE RENDERIZADORES", privacy: "Local primero · sin transporte de red · los fotogramas permanecen en el dispositivo" },
  zh: { live: "实时视觉系统", route: ["生成", "传输", "验证"], routeDetail: ["一次性 128 位秘密", "互补双相光学载波", "Hamming 修复与 CRC 完整性"], proof: "工程验证能力", unique: "种结构独立渲染器", corpus: "项确定性可靠性语料", channels: "条跨信道像素路径", stack: "光学协议栈", layers: ["艺术层 · 生成式渲染器", "载波层 · 18×18 互补双相", "恢复层 · Hamming (12,8)", "完整性 · CRC-16/CCITT"], constellation: "渲染器星座", privacy: "本地优先 · 不经过网络 · 摄像头画面留在设备内" },
};
