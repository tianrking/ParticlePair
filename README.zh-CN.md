...
- 13 种可选视觉模式，分为宇宙、有机、几何、大气与合成五个系列；每个 renderer 家族严格只保留一个代表。
- 13 个结构独立的生成器：旋臂、光幕、涟漪、花瓣、轨道、节点、触手、云雾、编织、晶体、符文、城市与余烬。历史上的换色和参数变体不再作为新模式出现在公开目录中。
- 支持搜索、分类筛选、每模式独立三色光谱、自动展演、选择持久化和沉浸式传输舞台。
- 界面为每种模式解释生成算法、摄像头提取方法与鲁棒性策略。
- 确定性的 Visual Quality Engine 使用固定标准载波渲染，测量鲜艳度、对比度、色相覆盖、运动连续性与综合评分。目录完整性还强制要求每个 renderer 家族只能存在一个模式，防止换色或参数变体再次被包装成新视觉。
...
- Mission Control 用独立吸附的左侧控制轨平衡不断增长的发送实验室：展示实时视觉身份与色板、生成→传输→验证流程、13/104/78 工程规模、光学协议栈、13 个可交互 renderer 快捷入口和本地优先隐私声明。叙事标题会正常滚走，控制轨固定在 94px；低于 1050px 时收起深层控件并恢复普通纵向排版。
...
- Environmental Mode Oracle 会让全部 13 种结构独立模式通过用户选择的合成摄像头信道，并按实测结果排名。精确恢复秘密并通过 CRC 是硬性前提；通过者再按同步质量与 Hamming 修复数量排序。
- Universal Channel Atlas 会让 13 种渲染器全部通过 6 种合成摄像头信道，共执行 78 条真实像素路径。Minimax 排名依次优先保证完整 CRC 覆盖、最差信道质量、平均质量和 Hamming 修复成本。6×13 光谱场能暴露脆弱模式。
...
- Reliability Marathon 使用固定的 8 个互不相同的 128 位秘密遍历全部 13 个渲染器，共执行 104 次真实双相 Canvas 评估。8×13 热图、通过数和最低同步质量可在不同浏览器间复现。
- Evidence Seal 会导出规范化的 `particlepair-reliability/v2` JSON，其中包含 104 个质量结果和 SHA-256 摘要。V2 明确标识去重后的 8×13 语料，不会与旧 8×50 证据混淆。
- Local Evidence Inspector 严格检查 104 项语料、结构和隐私约束，并在本机重新计算汇总与 SHA-256。
- Reliability Constellation 逐项比较两份已验真的 104 格记录；两个百分点死区过滤取整噪声，CRC 通过/失败变化始终优先。
