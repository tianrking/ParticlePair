...
- 确定性的 Visual Quality Engine 使用固定标准载波渲染，测量鲜艳度、对比度、色相覆盖、运动连续性与综合评分，并将 8×8 色彩—运动指纹和同一渲染家族中最相近的模式比较。当前 50 模式基线为最低 64、平均 73、最低差异度 49；质量低于 60 或差异度低于 40 时审计直接失败。
...
- Studio Capsule 只把经过白名单允许的呈现设置——模式、协议版本、v2 驻留时间、调制强度和渲染预算——序列化为带版本的分享链接。解码器会拒绝未知模式和越界参数；生成链接时会清除原有全部查询参数与片段，因此秘密、Session ID、SAS 和摄像头数据都不会进入 Capsule。
- Environmental Mode Oracle 会让全部 50 种模式通过用户选择的合成摄像头信道，并按实测结果排名。精确恢复秘密并通过 CRC 是硬性前提；通过者再按同步质量与 Hamming 修复数量排序，最后才用目录顺序稳定破同分。前三名可以直接应用。
- Universal Channel Atlas 会让 50 种渲染器全部通过 6 种合成摄像头信道，共执行 300 条真实像素路径。Minimax 排名依次优先保证完整 CRC 覆盖、最差信道质量、平均质量和 Hamming 修复成本。6×50 光谱场能暴露脆弱模式，并可立即应用跨环境最稳健的前三种渲染器。
- Phase-Safe Choreography 将自动场景切换对齐到完整的 600ms A+B 双相边界，把装饰突变限制在已知双相边界，而不是任意发生在一个符号内部。Phase Chronograph 通过直接更新 DOM 显示实时 A/B 相位和剩余毫秒，不会以动画帧率触发整个 React 页面重渲染。
- Immersive Receiver 提供适合单手使用的全屏摄像头界面，复用同一个扫描器，只镜像视频流用于预览。它组合了遮挡感知四角、紧凑 Confidence Aurora、针对当前瓶颈的物理操作指导、Escape/可见关闭按钮和可选 Wake Lock。界面可以在授权前打开，因此启动摄像头仍然必须由用户明确操作。
- Lab Dock 将兼容性矩阵、摄像头信道、环境 Oracle 和审美审计收进一个四段式工作台。未激活模块仍保持挂载，因此已完成结果不会丢失或重复计算；任一实验运行期间都会锁定导航，避免进度被隐藏或产生歧义。
- Reliability Marathon 使用固定的 8 个互不相同的 128 位秘密遍历全部 50 个渲染器，共执行 400 次真实双相 Canvas 评估，包含精确恢复、Hamming 与 CRC。8×50 热图、通过数和最低同步质量可在不同浏览器间复现。批量工具只跳过 PNG 证据编码，像素采样器与解码器完全相同。
- Evidence Seal 会导出规范化的 `particlepair-reliability/v1` JSON，其中包含 400 个取整后的质量结果、配置、汇总、明确的隐私标记和 SHA-256 摘要；校验可以发现文件修改。Seal 被明确标注为“仅完整性”：它不是数字签名、可信时间戳、设备证明，也不能证明测试由独立第三方执行。
- Local Evidence Inspector 可在浏览器内直接导入该 JSON，严格检查 400 项语料、结构和隐私约束，重新计算汇总与规范化 SHA-256，并清楚区分完整、被修改及格式错误的记录。文件始终留在本机；它仍是完整性校验，而不是身份认证。
- Reliability Constellation 会逐项比较两份已经验真的记录。两个百分点的死区用于过滤取整噪声，而准确解码的通过/失败变化始终拥有最高优先级。400 格光谱图展示改善、稳定及退化项，并按每种模式的 8 个密钥聚合，列出退化证据最强的三种视觉渲染器。
- Camera Channel Lab 覆盖正常、弱光、曝光漂移、失焦、传感器噪声与局部遮挡。只有恢复秘密完全一致并通过 CRC 才计为成功。
- Adaptive Signal Architect 会让当前视觉的 7 档调制强度全部通过 6 种摄像头信道，共执行 42 条真实像素路径。它选择能够完整通过 CRC 且最差信道质量不低于 47% 的最低强度，再增加最多 8 个百分点安全余量。7×6 光谱矩阵会保留较低强度的失败证据，而不是隐藏校准过程。
...
Verification Constellation 将这条边界变成五节点验证仪式：光学恢复、CRC、独立 SAS 派生、物理屏幕核对和本地接受。在两端本地 SAS 一致前，`WORDS MATCH` 始终禁用；SAS 不一致时无法强行接受，`REJECT` 会进入终止警告状态。接受仅代表记录了本地人工确认，生产流程仍必须继续执行经过审计的认证密钥交换。
