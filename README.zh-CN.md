...
- 确定性的 Visual Quality Engine 使用固定标准载波渲染，测量鲜艳度、对比度、色相覆盖、运动连续性与综合评分，并将 8×8 色彩—运动指纹和同一渲染家族中最相近的模式比较。当前 50 模式基线为最低 60、平均 68、最低差异度 51；质量低于 60 或差异度低于 40 时审计直接失败。
